# -*- coding: utf-8 -*-
"""SDM Updated  (DROPNA).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1wOkceY6VoH_Z2ArUpre3QuaYX2byyXrn
"""
...
# Menghapus seluruh bentuk NaN, Infinity pada Dataframe
...
# Data Set Bersih
df.to_csv('SHARP_SDO_Data_20112015_Drop_DataBersih.csv', index=False)
...
df.to_csv('SHARP_SDO_Data_20112015_Drop_DataBersih_Class.csv', index=False)
...
df.to_csv('SHARP_SDO_Data_20112015_Drop_Data Bersih_Without Duplicates.csv', index=False)
...
# Group the data by date and flare class, and count the number of occurrences
flare_count = df.groupby(['T_REC', 'CLASS']).size().reset_index(name='counts')

# Pivot the data so that each class has its own column
flare_count_pivot = flare_count.pivot(index='T_REC', columns='CLASS', values='counts').fillna(0)

# Create the heatmap
sns.heatmap(flare_count_pivot, cmap='Reds')

# Set the title and axis labels
plt.title("Solar Flare Classification", fontsize=16, fontweight='bold')
plt.xlabel("Flare Class", fontsize=14, fontweight='bold')
